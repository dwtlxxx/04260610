...
/* 左侧栏现在是"图标轨道 + 悬停展开"（竖排图标在收起态仍然可见），
   顶栏的开关按钮与早先的隐形热区都已删除。
   悬停效果本身由 CSS 实现，桩里没有样式引擎，所以这里断言【结构上必须成立】的事：
     ① 旧的手动开关与它的 action 已彻底移除；
     ② 旧的隐形热区元素已删除（它在窗口化时点不到，是这次要修的问题）；
     ③ 轨道态要保留的图标（.nav-board-icon）与要靠 CSS 淡出的文字（.nav-label）都在。
...
check('已删除隐形热区元素（改为可见轨道触发）', !/sidebar-hotzone/.test(deskHtml));
check('侧栏已渲染，且轨道态要保留的图标存在', /class="sidebar"/.test(deskHtml) && /nav-board-icon/.test(deskHtml));
check('文字信息带 nav-label 标记（CSS 靠它在轨道态淡出、悬停时淡入）',
  (deskHtml.match(/class="nav-label"/g) || []).length >= 5,
  `nav-label 出现 ${(deskHtml.match(/class="nav-label"/g) || []).length} 次`);
